...
from PyQt5.QtWidgets import (
    QWidget, QVBoxLayout, QLabel, QTabWidget, QGroupBox, QHBoxLayout, QTextEdit,
    QProgressBar, QFrame, QGridLayout, QScrollArea, QTableWidget, QTableWidgetItem,
...
class SummaryWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._update_threshold = 0.3  # Reduced threshold for better responsiveness
    ...
    def _log_message(self, message: str, level: str = "INFO"):
        ...
        try:
            if hasattr(self, 'parent_window') and self.parent_window and hasattr(self.parent_window, '_log_message'):
    ...
    def update_summary(self, results: Optional[List[DimensionalResult]] = None, 
                      table_data: Optional[pd.DataFrame] = None, force_refresh: bool = False):
        """Update summary with comprehensive error handling"""
        try:
            now = datetime.now()
    ...
    def _analyze_evaluation_types(self, results: List[DimensionalResult]):
        """Analyze results by evaluation type"""
        eval_stats = {}
        for r in results:
            etype = getattr(r, 'feature_type', 'Normal') or "Normal"
            if etype not in eval_stats:
                eval_stats[etype] = {
                    'count': 0, 'passed': 0, 'failed': 0, 'warning': 0, 'not_evaluable': 0
                }
    ...
    def _analyze_measurements(self, results: List[DimensionalResult]):
        ...
        measurement_stats['avg_measurements_per_dimension'] = total_meas / len(results) if results else 0
    ...
    def _update_with_table_data(self, table_data: pd.DataFrame):
        ...
        self.metrics['completeness'] = (total_measurements / total_possible) * 100 if total_possible else 0
    ...
    def _create_overview_tab(self):
        ...
        session_frame.setStyleSheet("background-color: #2c3e50; color: white; border-radius: 8px; padding: 8px;")
        session_layout = QHBoxLayout()
        
        session_time = datetime.now() - self.session_start_time
        session_info = QLabel(f"🕐 Session Time: {str(session_time).split('.')[0]} | 📅 Started: {self.session_start_time.strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    def _create_quality_tab(self):
        ...
        self.failed_table.setHorizontalHeaderLabels(["Element ID", "Description", "Cavity", "Out of Spec"])
        ...
        if m['cavity_breakdown']:
            for cavity, stats in m['cavity_breakdown'].items():
                if stats['total'] > 0:
                    ...
                    info_label = QLabel(f"🏭 Cavity {cavity}: {stats['passed']}/{stats['total']} passed")
                    info_label.setFont(QFont("Segoe UI", 9))
                    
                    rate_label = QLabel(f"{success_rate:.1f}%")
                    rate_label.setFont(QFont("Segoe UI", 9, QFont.Bold))
                    rate_label.setStyleSheet(f"color: {'#27ae60' if success_rate >= 95 else '#e74c3c' if success_rate < 80 else '#f39c12'};")
    ...
    def _update_quality_tab_content(self):
        ...
        for i, dim in enumerate(m['failed_dimensions']):
            ...
            self.failed_table.setItem(i, 3, QTableWidgetItem(str(dim['out_of_spec_count'])))
    ...
    def _update_integrity_tab_content(self):
        ...
        meas_info = [
            ...
            ("Avg per Dimension", f"{meas_stats.get('avg_measurements_per_dimension', 0):.1f}", "📊"),
            ("Full Measurement Sets", str(meas_stats.get('dimensions_with_full_measurements', 0)), "✅"),
            ("Out of Spec Measurements", str(meas_stats.get('out_of_spec_measurements', 0)), "❌"),
        ]
        ...
        tol_info = [
            ...
            ("Bilateral Tolerances", str(tol_stats.get('bilateral_tolerances', 0)), "⚖️"),
            ("Unilateral Tolerances", str(tol_stats.get('unilateral_tolerances', 0)), "➡️"),
    ...
    def _update_performance_tab_content(self):
        ...
        if m['study_history']:
            ...
            history_table.setRowCount(min(10, len(m['study_history'])))  # Show last 10 studies
            history_table.setMaximumHeight(300)
            history_table.setAlternatingRowColors(True)
            
            for i, study in enumerate(m['study_history'][-10:]):
                timestamp = study['timestamp'].strftime('%H:%M:%S') if isinstance(study['timestamp'], datetime) else str(study['timestamp'])
                ...
                history_table.setItem(i, 5, QTableWidgetItem(f"{study['success_rate']:.1f}%"))
    ...
    def _update_detailed_analysis_tab_content(self):
        ...
        # Update evaluation type breakdown
        self._clear_layout(self.evaltype_layout)
        
        if m['evaltype_breakdown']:
            eval_table = QTableWidget()
            eval_table.setColumnCount(6)
            eval_table.setHorizontalHeaderLabels([
                "Evaluation Type", "Count", "Passed", "Failed", "Warnings", "Success Rate"
            ])
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if m['success_rate'] < 95 and m['success_rate'] >= 80:
            recommendations.append("⚠️ Success rate below 95% - Review failed dimensions and consider process improvements.")
        elif m['success_rate'] < 80:
            recommendations.append("🚨 CRITICAL: Success rate below 80% - Immediate process review and corrective actions required.")
        
        # Completeness recommendations
        if m['completeness'] < 90:
            recommendations.append("📊 Incomplete measurement data detected - Consider adding more measurement points for better statistical analysis.")
        elif m['completeness'] < 70:
            recommendations.append("🚨 LOW DATA COMPLETENESS: Less than 70% measurement coverage - Data quality may be compromised.")
        
        # Failed dimensions recommendations
        if m['failed']:
            recommendations.append(f"❌ {m['failed']} dimensions failed - Prioritize investigation of root causes for failed measurements.")
        ...
        if cavity_issues:
            recommendations.append(f"🏭 Poor performance in cavities: {', '.join(cavity_issues)} - Check tooling and process parameters.")
        
        # Measurement recommendations
        meas_stats = m.get('measurement_stats', {})
        if meas_stats.get('avg_measurements_per_dimension', 0) < 3:
            recommendations.append("📏 Low measurement count per dimension - Consider increasing sample size for better statistical confidence.")
        
        # Tolerance recommendations
        tol_stats = m.get('tolerance_analysis', {})
        if tol_stats.get('without_tolerances', 0) > 0:
            recommendations.append(f"📐 {tol_stats['without_tolerances']} dimensions without tolerances - Review specification requirements.")
        
        # Warning dimensions
        if m['warning']:
            recommendations.append(f"⚠️ {m['warning']} dimensions with warnings - Review warning conditions and consider corrective actions.")
        
        # Default positive feedback
        if not recommendations:
            recommendations.append("✅ Excellent! All quality metrics are within acceptable ranges. Continue maintaining current standards.")
    ...
    def record_edit(self, description: str):
        ...
        self.metrics['edit_history'].append({
