# src/services/dim_data_export_service.
import json
import csv
import os
import logging
...
class DataExportService:
    ...
    def save_to_json(results: List[DimensionalResult], filepath: str) -> None:
        ...
        try:
            ...
            logging.getLogger(__name__).info(f"Successfully exported {len(results)} results to JSON: {filepath}")
        ...
        try:
            ...
            for result in results:
                ...
                if isinstance(result_dict.get('measurements'), list):
                    result_dict['measurements'] = '; '.join(f"{m:.4f}" for m in result_dict['measurements'])
                
                if isinstance(result_dict.get('deviation'), list):
                    result_dict['deviation'] = '; '.join(f"{d:.4f}" for d in result_dict['deviation'])
            ...
            logging.getLogger(__name__).info(f"Successfully exported {len(results)} results to CSV: {filepath}")
            
        except Exception as e:
            logging.getLogger(__name__).error(f"Failed to save CSV file {filepath}: {str(e)}")
            raise

    def export_results(self, results: List[DimensionalResult], export_dir: str, base_filename: str) -> Tuple[str, str]:
        """
        ...
        summary_path = os.path.join(export_dir, f"{base_filename}_{timestamp}_summary.txt")
        self._generate_summary_report(results, summary_path)
        
        self.logger.info(f"Export completed: JSON={json_path}, CSV={csv_path}, Summary={summary_path}")
        
        return json_path, csv_path

    def _generate_summary_report(self, results: List[DimensionalResult], filepath: str) -> None:
        """
            ...
            with open(filepath, 'w', encoding='utf-8') as f:
                ...
                if failed_elements:
                    ...
                    for elem in failed_elements:
                        f.write(f"- {elem.element_id}: {elem.description}\n")
                        f.write(f"  Batch: {elem.batch}, Cavity: {elem.cavity}\n")
                        f.write(f"  Out of spec measurements: {elem.out_of_spec_count}/{len(elem.measurements)}\n")
                        if elem.warnings:
                ...
                if bad > 0:
                    ...
                    f.write("- Check measurement procedures and equipment calibration\n")
                    f.write("- Consider process adjustments for out-of-spec elements\n")
                
                if warning > 0:
                    f.write("- Address warning conditions to improve measurement reliability\n")
                    f.write("- Ensure sufficient number of measurements per element\n")
                
                if success_rate < 95:
                    f.write("- Overall success rate below 95% - comprehensive review recommended\n")
    ...
    def load_from_json(filepath: str) -> List[DimensionalResult]:
        ...
        try:
            ...
            logging.getLogger(__name__).info(f"Successfully loaded {len(results)} results from JSON: {filepath}")
            return results
