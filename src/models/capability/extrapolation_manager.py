...
class ExtrapolationConfig:
    ...
    def __post_init__(self):
        if self.available_sizes is None:
            self.available_sizes = [50, 60, 70, 80, 90, 100, 125]
...
class ExtrapolationManager:
    ...
    def _calculate_anderson_darling_manual(self, sample: np.ndarray, 
    ...
    def extrapolate_sample(self, original_sample: List[float], 
                          nominal: float, 
                          tol_minus: float,
                          target_size: Optional[int] = None,
                          interactive: bool = True) -> ExtrapolationResults:
        """
        ...
        if target_size <= n_orig:
            print("Target size is not larger than original sample. No extrapolation performed.")
            return ExtrapolationResults(
        ...
        while attempt < self.config.max_attempts:
            ...
            if p_value >= self.config.target_p_value:
                print(f"Success! Sample is probably normal (p = {p_value:.4f} >= {self.config.target_p_value})")
                return ExtrapolationResults(
        ...
        print(f"Warning: Could not achieve p >= {self.config.target_p_value} after {self.config.max_attempts} attempts.")
        print(f"Best result: A² = {best_ad_stat:.4f}, p ≈ {best_p_value:.4f}")
        
        return ExtrapolationResults(
            extrapolated_values=best_sample.tolist() if best_sample is not None else original_sample.tolist(),
            ad_statistic=best_ad_stat,
            p_value=best_p_value,
            was_extrapolated=True
        )
    
    def extrapolate_multiple_samples(self, samples_data: List[dict],
                                   interactive: bool = True) -> List[ExtrapolationResults]:
        """
        ...
        for sample_data in samples_data:
            try:
                ...
                results.append(ExtrapolationResults(
                    extrapolated_values=[],
                    ad_statistic=0.0,
                    p_value=0.0,
                    was_extrapolated=False
    ...
    def batch_extrapolate(self, samples_data: List[dict],
                         target_size: int) -> List[ExtrapolationResults]:
        """
