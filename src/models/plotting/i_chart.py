...
class IChart(SPCChartBase):
    ...
    def _parse_element_data(self):
        ...
        try:
            ...
            if len(self.tolerance) != 2:
                raise ValueError(
                    "tolerance must be a list of [lower_tolerance, upper_tolerance]"
                )
            ...
            self.logger.debug(
                f"Parsed data: n={len(self.sample_data)}, nominal={self.nominal}, LSL={self.lsl}, USL={self.usl}"
            )
        ...
        try:
            ...
            if self.moving_range is None:
                if len(self.sample_data) < 2:
                    raise ValueError(
                        "Need at least 2 data points to calculate moving range"
                    )
            ...
            if self.ucl is None or self.lcl is None:
                ...
                self.logger.debug(
                    f"Calculated control limits: UCL={self.ucl:.4f}, LCL={self.lcl:.4f}"
                )
    ...
    def _calculate_process_capability(self):
        ...
        try:
            if self.cp is None or self.cpk is None:
                if len(self.sample_data) < 2:
                    self.logger.warning(
                        "Need at least 2 data points for process capability calculation"
                    )
                    return
                ...
                self.logger.debug(
                    f"Calculated process capability: Cp={self.cp:.4f}, Cpk={self.cpk:.4f}"
                )
    ...
    def plot(self):
        ...
        try:
            ...
            ax.axhline(
                self.usl,
                color=self.COLOR_TARONJA,
                linestyle="--",
                linewidth=1,
                alpha=0.8,
                zorder=1,
                label=self.labels.get("usl_label", "USL = {usl:.3f}").format(
                    usl=self.usl
                ),
            )
            ax.axhline(
                ...
                alpha=0.8,
                zorder=1,
                label=self.labels.get("lsl_label", "LSL = {lsl:.3f}").format(
                    lsl=self.lsl
                ),
            )
