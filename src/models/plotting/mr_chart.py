    ...
    def _parse_element_data(self):
        ...
        try:
            ...
            if len(self.sample_data) < 2:
                raise ValueError(
                    "At least 2 data points are required for Moving Range chart"
                )
            ...
            self.logger.debug(
                f"Parsed data: n={len(self.sample_data)}, element={self.element}"
            )
    ...
    def _calculate_moving_ranges(self):
        ...
        try:
            ...
            if len(self.moving_ranges) != len(self.indices):
                raise ValueError(
                    f"Moving ranges length ({len(self.moving_ranges)}) must match indices length ({len(self.indices)})"
                )
    ...
    def _calculate_control_limits(self):
        ...
        try:
            ...
            if self.mr_ucl is None or self.mr_lcl is None:
                ...
                self.logger.debug(
                    f"Calculated MR control limits: UCL={self.mr_ucl:.4f}, LCL={self.mr_lcl:.4f}"
                )
    ...
    def _calculate_estimated_sigma(self):
        ...
        try:
            if self.estimated_sigma is None:
                ...
                self.logger.debug(
                    f"Calculated estimated sigma: {self.estimated_sigma:.4f}"
                )
    ...
    def plot(self):
        ...
        try:
            ...
            ax.axhline(
                ...
                linewidth=1.5,
                zorder=2,
                label=self.labels.get("ucl_label", "UCL = {ucl:.3f}").format(
                    ucl=self.mr_ucl
                ),
            )
            ax.axhline(
                ...
                linewidth=1.5,
                zorder=2,
                label=self.labels.get("cl_label", "CL = {cl:.3f}").format(
                    cl=self.mr_mean
                ),
            )
            ax.axhline(
                ...
                linewidth=1.5,
                zorder=2,
                label=self.labels.get("lcl_label", "LCL = {lcl:.3f}").format(
                    lcl=self.mr_lcl
                ),
            )

            # Set titles and labels
            chart_title = self.labels.get(
            ...
            if out_of_control:
                for idx in out_of_control:
                    ax.annotate(
                        "!",
                        xy=(self.indices[idx], self.moving_ranges[idx]),
                        xytext=(5, 5),
                ...
                self.logger.warning(
                    f"Found {len(out_of_control)} out-of-control points"
                )
...
if __name__ == "__main__":
    ...
    try:
        ...
        chart = MRChart(
            input_json_path=tmp_path, lang="ca", show=True, element_name="test_element"
        )
