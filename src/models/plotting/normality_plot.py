...
class NormalityAnalysisChart(SPCChartBase):
    def __init__(
        ...
        **kwargs,
        ...
        self.logger.info(
            f"Initializing NormalityAnalysisChart for element: {self.element_name}"
        )
        ...
        try:
            ...
            self.logger.info(
                f"Loaded {len(self.mostra)} sample points for element {self.element}"
            )

            # Validate required data
            if len(self.mostra) == 0:
                raise ValueError(
                    f"No sample data found for element {self.element_name}"
                )

        except Exception as e:
            self.logger.error(
                f"Error initializing NormalityAnalysisChart: {e}", exc_info=True
            )
            raise
    ...
    def plot(self):
        ...
        try:
            ...
            self.logger.debug(
                f"Statistics - Mean: {mean:.4f}, Std: {std:.4f}, LSL: {lsl:.4f}, USL: {usl:.4f}"
            )
            ...
            self.logger.error(
                f"Error creating normality analysis chart: {e}", exc_info=True
            )
            raise
    ...
    def _plot_histogram_kde(self, ax, mean, std, lsl, usl, tr):
        ...
        ax.axvline(
            lsl, color=self.COLOR_VERMELL, linestyle="-", linewidth=1.2, alpha=0.8
        )
        ax.axvline(
            usl, color=self.COLOR_VERMELL, linestyle="-", linewidth=1.2, alpha=0.8
        )
        ax.axvline(
            self.nominal, color=self.COLOR_VERD, linestyle="-", linewidth=1.2, alpha=0.8
        )
        ax.axvline(
            mean, color=self.COLOR_NEGRE, linestyle="--", linewidth=1.2, alpha=0.8
        )
        ...
        label_props = dict(
            ...
            va="bottom",
            bbox=dict(
                facecolor="white", edgecolor="none", boxstyle="round,pad=0.2", alpha=0.8
            ),
        )
