...
class CapabilityChart(SPCChartBase):
    ...
    def __init__(self, input_json_path, element_name=None, **kwargs):
        ...
        self.logger.info(f"Initialized CapabilityChart for element: {self.element_name}")
    ...
    def _get_chart_data(self):
        ...
        self.logger.debug(f"Chart data extracted: mean={data['mean']:.4f}, "
    ...
    def _calculate_scale_parameters(self, data):
        ...
        x_points = [
            data['lsl'], data['usl'],
            data['mean'] - 3 * data['std_short'], data['mean'] + 3 * data['std_short'],
        ...
        self.logger.debug(f"Scale parameters calculated: min={scale_min:.4f}, max={scale_max:.4f}")
    ...
    def _draw_main_chart(self, ax, data, translations):
        ...
        # Add row labels
        for y_pos, label in zip([2, 1, 0], [translations['long_term'], translations['short_term'], translations['specifications']]):
            ax.text(scale_min - 0.01 * abs(scale_max - scale_min), y_pos, label,
                   verticalalignment='center', fontsize=10, fontweight='normal',
                   rotation=90, ha='right', fontname=self.FONT_NAME)
        
        # Draw horizontal lines
        ax.hlines(y=0, xmin=data['lsl'], xmax=data['usl'], color=self.COLOR_VERMELL, linestyle='-', linewidth=1)
        ax.hlines(y=1, xmin=data['mean'] - 3 * data['std_short'], xmax=data['mean'] + 3 * data['std_short'], 
                 color=self.COLOR_BLAU, linestyle='-', linewidth=1)
        ax.hlines(y=2, xmin=data['mean'] - 3 * data['std_long'], xmax=data['mean'] + 3 * data['std_long'], 
                 color=self.COLOR_BLAU, linestyle='-', linewidth=1)
        
        # Add markers
        ax.plot([data['lsl'], data['usl']], [0, 0], 's', color=self.COLOR_VERMELL, markersize=5, zorder=6)
        ax.plot([data['mean'] - 3 * data['std_short'], data['mean'] + 3 * data['std_short']], [1, 1], 
               '+', color=self.COLOR_BLAU, markersize=7, zorder=6)
        ax.plot([data['mean'] - 3 * data['std_long'], data['mean'] + 3 * data['std_long']], [2, 2], 
               '+', color=self.COLOR_BLAU, markersize=7, zorder=6)
        ...
        # Add vertical lines
        ax.vlines(data['mean'], -0.3, 2.7, color=self.COLOR_NEGRE, linestyle='-.', linewidth=0.75, zorder=5)
        ax.vlines(data['usl'], -0.5, 2.7, color=self.COLOR_VERMELL, linestyle='--', linewidth=0.75)
        ax.vlines(data['lsl'], -0.5, 2.7, color=self.COLOR_VERMELL, linestyle='--', linewidth=0.75)
        
        # Add limit labels
        ax.text(data['usl'], 2.5, translations['usl_label'].format(usl=data['usl']), 
               color=self.COLOR_VERMELL, fontsize=9, ha='center', va='bottom',
               fontname=self.FONT_NAME,
               bbox=dict(facecolor='white', edgecolor='none', boxstyle='round,pad=0.1'))
        ax.text(data['lsl'], 2.5, translations['lsl_label'].format(lsl=data['lsl']), 
               color=self.COLOR_VERMELL, fontsize=9, ha='center', va='bottom',
               fontname=self.FONT_NAME,
               bbox=dict(facecolor='white', edgecolor='none', boxstyle='round,pad=0.1'))
        ...
        ax.set_yticks([])
        ax.set_xlabel(translations['value_measured'], fontsize=10, fontname=self.FONT_NAME)
        ax.grid(True, axis='both', linestyle='--', alpha=0.4)
        
        # Add title and subtitle
        title = translations['process_capability_plot'].format(element=self.element_name)
        ...
        ax.text(0.5, 1.02, subtitle, transform=ax.transAxes, ha='center',
               fontsize=11, color="#444444", va='bottom', fontname=self.FONT_NAME)
    ...
    def _create_capability_table(self, axbox, data, translations, table_type='short'):
        ...
        header_text = translations['short_term'] if is_short else translations['long_term']
        header_color = "#e6e6f2" if is_short else "#d9f2d9"
        cell_color = "#f9f9f9" if is_short else "#f9fff9"
        
        # Data selection
        if is_short:
            indices = {'primary': 'cp', 'secondary': 'cpk', 'ppm': 'ppm_short', 'std': 'std_short'}
            labels = {'primary': 'Cp', 'secondary': 'Cpk'}
        else:
            indices = {'primary': 'pp', 'secondary': 'ppk', 'ppm': 'ppm_long', 'std': 'std_long'}
        ...
        # Add cells
        table.add_cell(0, 0, width=0.5, height=0.25, text=header_text, loc='center', facecolor=header_color)
        table.add_cell(1, 0, width=wd, height=0.2, text=labels['primary'], loc='left', facecolor=cell_color)
        table.add_cell(1, 1, width=wd, height=0.2, text=f"{data[indices['primary']]:.2f}", loc='left')
        table.add_cell(2, 0, width=wd, height=0.2, text=labels['secondary'], loc='left', facecolor=cell_color)
        table.add_cell(2, 1, width=wd, height=0.2, text=f"{data[indices['secondary']]:.2f}", loc='left')
        table.add_cell(3, 0, width=wd, height=0.2, text='PPM', loc='left', facecolor=cell_color)
        table.add_cell(3, 1, width=wd, height=0.2, text=f"{data[indices['ppm']]:.2f}", loc='left')
        table.add_cell(4, 0, width=wd, height=0.2, text=translations['std_dev'], loc='left', facecolor=cell_color)
        table.add_cell(4, 1, width=wd, height=0.2, text=f"{data[indices['std']]:.4f}", loc='left')
...
if __name__ == "__main__":
    ...
    test_data = {
        "test_diameter": {
            ...
            "std_short": 0.012,
            "std_long": 0.018,
            "tolerance": [-0.05, 0.05],  # LSL = 9.95, USL = 10.05
            "nominal": 10.0,
            "cp": 1.39,
            "cpk": 1.16,
            "pp": 0.93,
        ...
        "test_length": {
            ...
            "std_short": 0.025,
            "std_long": 0.035,
            "tolerance": [-0.1, 0.1],  # LSL = 24.9, USL = 25.1
            "nominal": 25.0,
            "cp": 1.33,
            "cpk": 1.24,
            "pp": 0.95,
    ...
    print("Testing CapabilityChart with sample data...")
    print(f"Created test data file: {temp_json_path}")
    
    try:
        # Test with first element (test_diameter)
        print("\n=== Testing with test_diameter element ===")
        chart1 = CapabilityChart(
            input_json_path=temp_json_path,
            element_name="test_diameter",
            lang="en",  # Using English for test
            show=True,
            save_path="test_capability_diameter.png"
        )
        chart1.plot()
        
        # Test with second element (test_length)
        print("\n=== Testing with test_length element ===")
        chart2 = CapabilityChart(
            input_json_path=temp_json_path,
            element_name="test_length",
            lang="en",
            show=True,
            save_path="test_capability_length.png"
        )
        chart2.plot()
        
        print("\n=== All tests completed successfully! ===")
        print("Generated files:")
        print("- test_capability_diameter.png")
        print("- test_capability_length.png")
