...
class ExtrapolatedSPCChart(SPCChartBase):
    ...
    def __init__(
        ...
        self.logger.info(f"ExtrapolatedSPCChart initialized for element: {self.element_name}")
    ...
    def _plot_normal_curve(self, ax, x: np.ndarray, normal_scaled: np.ndarray, data: Dict[str, Any]) -> None:
    ...
    def _plot_control_limits(self, ax, data: Dict[str, Any]) -> None:
        ...
        if data['lsl'] != data['nominal']:
            ax.axvline(data['lsl'], color=self.COLOR_VERMELL, linestyle='-', linewidth=0.75)
            ax.text(
                data['lsl'], ymax * 0.94, f'LSL ({data["lsl"]:.2f})', 
                color=self.COLOR_VERMELL, fontsize=9, fontname=self.FONT_NAME,
                ha='center', va='bottom',
        ...
        # Nominal value
        ax.axvline(data['nominal'], color=self.COLOR_VERD, linestyle='-', linewidth=0.75)
        ax.text(
            data['nominal'], ymax * 0.95, f'$x_{{0}}$ ({data["nominal"]:.2f})', 
            color=self.COLOR_VERD, fontsize=9, fontname=self.FONT_NAME,
            ha='center', va='bottom',
            bbox=dict(facecolor='white', edgecolor='none', boxstyle='round,pad=0.1')
        )
        ...
        if data['usl'] != data['nominal']:
            ax.axvline(data['usl'], color=self.COLOR_VERMELL, linestyle='-', linewidth=0.75)
            ax.text(
                data['usl'], ymax * 0.95, f'USL ({data["usl"]:.2f})', 
                color=self.COLOR_VERMELL, fontsize=9, fontname=self.FONT_NAME,
                ha='center', va='bottom',
                bbox=dict(facecolor='white', edgecolor='none', boxstyle='round,pad=0.1')
            )
    ...
    def _plot_rejection_zones(self, ax, x: np.ndarray, normal_scaled: np.ndarray, data: Dict[str, Any]) -> None:
        ...
        ax.fill_between(
            ...
            color=self.COLOR_VERMELL_CLAR, alpha=0.4
        )
    ...
    def _set_axis_limits(self, ax, x: np.ndarray, data: Dict[str, Any]) -> None:
        ...
        self.logger.debug(f"X-axis limits set: [{x_min_val - x_margin:.4f}, {x_max_val + x_margin:.4f}]")
    ...
    def _create_subtitle(self, data: Dict[str, Any]) -> str:
        ...
        if data['pp'] is not None and data['ppk'] is not None:
            pp_ppk_text = self.labels.get('subtitle_pp_ppk', 'Pp = {pp:.2f}, Ppk = {ppk:.2f}')
        ...
        if data['is_normal'] is not None:
            ...
            subtitle_parts.append(normal_text.format(
                normal=yes_text if data['is_normal'] else no_text
            ))
    ...
    def plot(self) -> None:
        ...
        try:
            ...
            title = self.labels.get('distribution_title', 'Measurement Distribution - {element}').format(
            ...
            if subtitle.strip():
                ax.text(
                    ...
                    fontsize=11, fontname=self.FONT_NAME, color="#444444",
            ...
            self.logger.error(f"Error creating extrapolated SPC chart: {e}", exc_info=True)
            raise
        finally:
...
if __name__ == "__main__":
    ...
    with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as f:
        ...
        example_json_path = f.name
    
    try:
        # Create and plot chart
        chart = ExtrapolatedSPCChart(
            input_json_path=example_json_path,
            lang="ca",
            show=True,
            element_name="element_example"
        )
        chart.plot()
        
    finally:
        # Clean up
        Path(example_json_path).unlink()