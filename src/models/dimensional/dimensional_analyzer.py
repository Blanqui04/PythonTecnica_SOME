# src/models/dimensional/dimensional_analyzer.
import os
import json
import csv
import logging
...
def _handle_single_tolerance(t: float, gdt_flags: Dict[str, bool], warnings: List[str]) -> tuple[float, float]:
    ...
    if gdt_flags.get("MIN") or gdt_flags.get("MMC") or gdt_flags.get("LMC") or gdt_flags.get("PROFILE"):
        ...
        upper_tol = abs(t)
        lower_tol = -abs(t)
        warnings.append("Single tolerance value interpreted as symmetric bilateral tolerance.")
...
def apply_bonus_tolerance(
    ...
    if ((gdt_flags.get("MMC") or gdt_flags.get("LMC")) 
        ...
        try:
            ...
            logging.getLogger(__name__).warning(f"Error calculating bonus tolerance: {str(e)}")
...
class DimensionalAnalyzer:
    ...
    def analyze_row(self, row: dict) -> DimensionalResult:
        ...
        try:
            ...
            if not raw_tol or (isinstance(raw_tol, list) and len(raw_tol) == 0):
                return self._create_no_tolerance_result(row, measurements, deviation, gdt_flags)
            ...
            if len(measurements) < 5:
                warnings.append(f"Only {len(measurements)} measurements provided; 5 or more recommended for statistical validity.")
            ...
            if measurements:
                near_lower = sum(1 for m in measurements if abs(m - lower_limit) < abs(upper_tol - lower_tol) * 0.1)
                near_upper = sum(1 for m in measurements if abs(m - upper_limit) < abs(upper_tol - lower_tol) * 0.1)
                
                if near_lower > len(measurements) * 0.5:
                    warnings.append("Many measurements near lower limit - process may be marginal")
                elif near_upper > len(measurements) * 0.5:
                    warnings.append("Many measurements near upper limit - process may be marginal")
            ...
            self.logger.error(f"Error analyzing row for element {row.get('element_id', 'Unknown')}: {str(e)}")
    ...
    def _create_no_tolerance_result(self, row: dict, measurements: List[float], deviation: List[float], gdt_flags: dict) -> DimensionalResult:
