# src/models/dimensional/measurement_validator.
import logging
        ...
        for i, measurement in enumerate(measurements):
            try:
                ...
                if not (-1e6 <= value <= 1e6):  # Reasonable range check
                    logger.warning(f"Measurement {i+1} value {value} is outside reasonable range")
                    continue
                valid_measurements.append(value)
            except (ValueError, TypeError):
                logger.warning(f"Invalid measurement value at position {i+1}: {measurement}")
                continue
        ...
        if datum_element_id is not None:
            ...
            if datum_measurement is not None:
                try:
                    ...
                    logger.warning(f"Invalid datum measurement value: {datum_measurement}")
...
def validate_batch_consistency(records: List[Dict[str, Any]]) -> List[str]:
    ...
    try:
        ...
        if len(batches) > 1:
            warnings.append(f"Multiple batch numbers found: {', '.join(map(str, batches))}")
                ...
                try:
                    ...
                    if eid in element_nominals:
                        if abs(element_nominals[eid] - nominal_val) > 1e-6:
                            warnings.append(f"Inconsistent nominal values for element {eid}")
                    else:
            ...
            if eid and tolerance is not None:
                if eid in element_tolerances:
                    if element_tolerances[eid] != tolerance:
                        warnings.append(f"Inconsistent tolerance values for element {eid}")
                else:
        ...
        for field in critical_fields:
            ...
            if missing_count > 0:
                warnings.append(f"{missing_count} records missing required field '{field}'")
        
        # Check for empty measurement lists
        empty_measurements = sum(1 for record in records 
                               if not record.get("measurements") or len(record.get("measurements", [])) == 0)
        ...
        single_measurement_count = sum(1 for record in records 
                                     if len(record.get("measurements", [])) == 1)
        if single_measurement_count > len(records) * 0.5:
            warnings.append(f"Over 50% of records have only single measurements - statistical analysis limited")
        ...
        for record in records:
            measurements = record.get("measurements", [])
            try:
                all_measurements.extend([float(m) for m in measurements if m is not None])
            except (ValueError, TypeError):
                pass
        ...
        if all_measurements:
            ...
            if range_val > 1000:
                warnings.append(f"Very large measurement range detected: {range_val:.2f}")
            ...
            if len(set(all_measurements)) == 1 and len(all_measurements) > 1:
                warnings.append("All measurements are identical - possible data entry error")
        
        # Check datum reference consistency
        datum_records = [record for record in records if record.get("datum_element_id")]
        if datum_records:
            datum_elements = set(record.get("datum_element_id") for record in datum_records)
            ...
            if missing_datums:
                warnings.append(f"Datum elements not found in records: {', '.join(missing_datums)}")
            ...
            for datum_id in datum_elements:
                ...
                for record in datum_records:
                    if record.get("datum_element_id") == datum_id:
                        ...
                        if record.get("datum_measurement") is not None:
                            try:
                                datum_measurements.add(float(record.get("datum_measurement")))
                            except (ValueError, TypeError):
                                pass
                ...
                if len(datum_measurements) > 1:
                    warnings.append(f"Inconsistent datum measurement values for {datum_id}")
...
def sanitize_record(record: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        # Clean string fields
        string_fields = ["element_id", "description", "batch", "cavity", "datum_element_id"]
        ...
        for field in numeric_fields:
            if field in sanitized and sanitized[field] is not None:
                try:
                    ...
                    logger.warning(f"Could not convert {field} to float: {sanitized[field]}")
                    sanitized[field] = None
                    ...
                    for i, measurement in enumerate(measurements):
                        if measurement is not None:
                            try:
                                ...
                                logger.warning(f"Skipping invalid measurement at position {i}: {measurement}")
                        else:
...
def validate_dimensional_result_compatibility(record: Dict[str, Any]) -> List[str]:
        ...
        if field not in record or record[field] is None:
            ...
            if field == "nominal" and isinstance(record[field], str):
                try:
                    ...
                    warnings.append(f"Field '{field}' cannot be converted to numeric type")
            else:
                warnings.append(f"Field '{field}' has incorrect type. Expected {expected_type.__name__}, got {type(record[field]).__name__}")
    ...
    if measurements:
        ...
        if non_numeric_count > 0:
            warnings.append(f"{non_numeric_count} measurements cannot be converted to numeric values")
    ...
    for field in optional_numeric_fields:
        if field in record and record[field] is not None:
            try:
                ...
                warnings.append(f"Optional field '{field}' cannot be converted to numeric type")
...
def prepare_record_for_analysis(record: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if compatibility_warnings:
        logger.warning(f"Compatibility issues for element {prepared.get('element_id', 'Unknown')}: {compatibility_warnings}")
