# src/models/dimensional/gdt_interpreter.py - ENHANCED FOR NOMINAL=0
import re
import logging
...
class GDTInterpreter:
    """Enhanced GD&T interpreter with better nominal=0 handling"""

    def __init__(self):
        self.logger = logging.getLogger(__name__)

        # Enhanced GD&T patterns with more aggressive matching for nominal=0 cases
        self.gdt_patterns = {
            # Position patterns - enhanced to catch more variations
            "position": re.compile(
                r"(?:position|true\s*position|tp|⌖)\s*[øΦ]?\s*([\d.,]+)\s*(\([MLS]\))?\s*([A-Z][\s|]*[A-Z]?[\s|]*[A-Z]?)?",
                re.IGNORECASE,
            ),
            
            # Flatness - very important for nominal=0
            "flatness": re.compile(
                r"(?:flatness|⏥)\s*[øΦ]?\s*([\d.,]+)(?:\s*([A-Z]))?", 
                re.IGNORECASE
            ),
            
            # Enhanced patterns for other GD&T types
    ...
    def parse_gdt_description(self, description: str) -> Dict[str, any]:
        """Enhanced GD&T parsing with better nominal=0 support"""
        gdt_info = {
            "has_gdt": False,
            "tolerance_type": None,
        ...
        try:
            ...
            for tolerance_type, pattern in self.gdt_patterns.items():
                ...
                if match:
                    ...
                    self.logger.debug(f"GD&T parsed: {tolerance_type} = {gdt_info['tolerance_value']}")
                    break

            # If no specific pattern matched, try generic GD&T detection
            if not gdt_info["has_gdt"]:
                # Try to extract any number that might be a tolerance
                number_pattern = re.compile(r"([\d.,]+)")
                numbers = number_pattern.findall(description)
                
                # Check for GD&T keywords without specific patterns
                gdt_keywords = [
                    "position", "flatness", "parallelism", "perpendicularity", 
                    "angularity", "profile", "circularity", "cylindricity",
                    "straightness", "runout", "concentricity", "symmetry"
                ]
                
                desc_lower = description.lower()
                for keyword in gdt_keywords:
                    if keyword in desc_lower and numbers:
                        gdt_info["has_gdt"] = True
                        gdt_info["tolerance_type"] = keyword
                        try:
                            # Take the first number as tolerance
                            gdt_info["tolerance_value"] = float(numbers[0].replace(",", "."))
                            gdt_info["warnings"].append(f"Generic {keyword} pattern matched")
                            self.logger.debug(f"Generic GD&T detected: {keyword} = {gdt_info['tolerance_value']}")
    ...
    def convert_gdt_to_tolerance_range(self, gdt_info: Dict, nominal: float) -> Tuple[float, float]:
        """Enhanced GD&T to tolerance conversion with proper nominal=0 handling"""
        ...
        self.logger.debug(f"Converting GD&T: {tolerance_type} = {tolerance_value}, nominal = {nominal}")
        ...
        if tolerance_type in ["flatness", "circularity", "cylindricity", "straightness"]:
            # These measure deviation from perfect form - always 0 to +tolerance
            self.logger.debug(f"Form tolerance: [0.0, {tolerance_value}]")
            return 0.0, tolerance_value

        # Position tolerance - for nominal=0, check absolute deviation
        elif tolerance_type == "position":
            if nominal == 0.0:
                # Position from theoretical exact location - bilateral around zero
                self.logger.debug(f"Position (nominal=0): [-{tolerance_value}, {tolerance_value}]")
                return -tolerance_value, tolerance_value
            else:
                # Position with non-zero nominal
                return -tolerance_value, tolerance_value

        # Orientation tolerances - bilateral around nominal
        elif tolerance_type in ["parallelism", "perpendicularity", "angularity"]:
            self.logger.debug(f"Orientation tolerance: [-{tolerance_value}, {tolerance_value}]")
            return -tolerance_value, tolerance_value

        # Profile tolerances - special handling
            ...
            if is_bilateral_profile:
                ...
                if nominal == 0:
                    # Profile from theoretical - usually bilateral
                    return -tolerance_value / 2, tolerance_value / 2
                else:
                    return -tolerance_value / 2, tolerance_value / 2

        # Runout tolerances - always unilateral positive
        elif tolerance_type in ["runout", "total_runout"]:
            return 0.0, tolerance_value

        # Concentricity and symmetry - bilateral around nominal
        elif tolerance_type in ["concentricity", "symmetry"]:
            return -tolerance_value, tolerance_value

        # Unknown GDT type - conservative bilateral
        else:
            self.logger.debug(f"Unknown GD&T type: [-{tolerance_value}, {tolerance_value}]")
            return -tolerance_value, tolerance_value
    ...
    def _validate_gdt_callout(self, gdt_info: Dict) -> List[str]:
        ...
        if material_condition:
            if tolerance_type in ["flatness", "circularity", "cylindricity"]:
                warnings.append(f"Material condition modifier not applicable to {tolerance_type}")

        # Check datum reference requirements
        if tolerance_type in ["position", "parallelism", "perpendicularity", "angularity", 
                             "profile", "concentricity", "symmetry", "runout", "total_runout"]:
