...
class GDTInterpreter:
    ...
    def __init__(self):
        ...
        self.gdt_patterns = {
            'position': re.compile(r'position\s+([\d.,]+)\s*(\([MLS]\))?\s*([A-Z]\s*[A-Z]?\s*[A-Z]?)?', re.IGNORECASE),
            'parallelism': re.compile(r'parallelism\s+([\d.,]+)\s*([A-Z]\s*[A-Z]?\s*[A-Z]?)?', re.IGNORECASE),
            'perpendicularity': re.compile(r'perpendicularity\s+([\d.,]+)\s*([A-Z]\s*[A-Z]?\s*[A-Z]?)?', re.IGNORECASE),
            'angularity': re.compile(r'angularity\s+([\d.,]+)\s*([A-Z]\s*[A-Z]?\s*[A-Z]?)?', re.IGNORECASE),
            'profile': re.compile(r'profile\s+([\d.,]+)\s*([A-Z]\s*[A-Z]?\s*[A-Z]?)?', re.IGNORECASE),
            ...
            'straightness': re.compile(r'straightness\s+([\d.,]+)\s*(\([MLS]\))?', re.IGNORECASE),
            'concentricity': re.compile(r'concentricity\s+([\d.,]+)\s*([A-Z]\s*[A-Z]?\s*[A-Z]?)?', re.IGNORECASE),
            'symmetry': re.compile(r'symmetry\s+([\d.,]+)\s*([A-Z]\s*[A-Z]?\s*[A-Z]?)?', re.IGNORECASE),
            'runout': re.compile(r'runout\s+([\d.,]+)\s*([A-Z]\s*[A-Z]?\s*[A-Z]?)?', re.IGNORECASE),
            'total_runout': re.compile(r'total\s+runout\s+([\d.,]+)\s*([A-Z]\s*[A-Z]?\s*[A-Z]?)?', re.IGNORECASE),
        }
        ...
        try:
            ...
            for tolerance_type, pattern in self.gdt_patterns.items():
                ...
                if match:
                    ...
                    try:
                        gdt_info['tolerance_value'] = float(match.group(1).replace(',', '.'))
                    except (ValueError, AttributeError):
                        gdt_info['warnings'].append(f"Could not parse tolerance value: {match.group(1)}")
    ...
    def _validate_gdt_callout(self, gdt_info: Dict) -> List[str]:
        ...
        if tolerance_type in ['position', 'parallelism', 'perpendicularity', 'angularity', 
            ...
            if not datum_references:
                ...
                warnings.append("Position tolerance typically requires at least 2 datum references")
...
def create_enhanced_gdt_flags(description: str) -> Dict[str, bool]:
    ...
    flags = {
        ...
        'PROFILE': gdt_info['tolerance_type'] in ['profile', 'profile_line', 'profile_surface'],
